...
const ManagementLOPDP = () => {
  ...
  const getCustomerById = useCallback(async (customerId, type) => {
    try {
      const response = await http.get(`customers/can-anonymize/${customerId}`);
      console.log("CLIENTE SELECIONADO ID:", customerId);
      console.log("Resultado de la verificacion: ", response.data);
      ...
      if (response.data.status === "success") {
        ...
        //setModalType(type);
        //openModal();
        if (response.data.analysisResult.canAnonymize === true) {
          alerts("Exito", "El cliente puede ser anonimizado", "success");
        } else if (response.data.analysisResult.canAnonymize === false) {
          alerts(
            "Alerta",
            `El cliente no puede ser anonimizado, porque tiene ${response.data.analysisResult.activePolicies} pólizas activas asociadas`,
            "warning"
          );
        } else {
          alerts(
            "Error",
            "No se pudo determinar si el cliente puede ser anonimizado",
            "error"
          );
        }
      } else {
...
export default ManagementLOPDP;
